...
func TestValidateClerkProvider(t *testing.T) {
	// Clerk provider does not require jwt_secret, but does require clerk_issuer.
	noIssuer := `{
		"server": {"addr": ":8080"},
		"auth": {"provider": "clerk"}
	}`
	path := writeTempConfig(t, noIssuer)
	_, err := Load(path)
	if err == nil {
		t.Fatal("expected error for clerk provider without clerk_issuer, got nil")
	}

	// Clerk provider with issuer should pass validation (jwt_secret not required).
	withIssuer := `{
		"server": {"addr": ":8080"},
		"auth": {"provider": "clerk", "clerk_issuer": "https://foo.clerk.accounts.dev"}
	}`
	path = writeTempConfig(t, withIssuer)
	cfg, err := Load(path)
	if err != nil {
		t.Fatalf("Load with clerk provider: %v", err)
	}
	if cfg.Auth.Provider != "clerk" {
		t.Errorf("Auth.Provider: got %q, want %q", cfg.Auth.Provider, "clerk")
	}
	if cfg.Auth.ClerkIssuer != "https://foo.clerk.accounts.dev" {
		t.Errorf("Auth.ClerkIssuer: got %q", cfg.Auth.ClerkIssuer)
	}
}
