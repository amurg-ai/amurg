...
import (
	...
	"github.com/amurg-ai/amurg/hub/api"
...
// Options contains optional dependencies injected by the caller (e.g. SaaS billing).
type Options struct {
	// BillingFactory creates billing service and enforcer from the store and config.
	// When nil, billing is disabled (community mode).
	BillingFactory func(store.Store, config.BillingConfig, *slog.Logger) (billing.Service, billing.Enforcer)
}
...
func New(cfg *config.Config, opts Options, logger *slog.Logger) (*Hub, error) {
	// Initialize storage.
	...
	var enforcer billing.Enforcer
	if cfg.Billing.Enabled && opts.BillingFactory != nil {
		billingSvc, enforcer = opts.BillingFactory(db, cfg.Billing, logger)
	}

	// Initialize API server.
	apiSrv := api.NewServer(db, authProvider, loginProvider, runtimeAuth, rt, cfg, api.ServerOptions{
		Billing:           billingSvc,
		Enforcer:          enforcer,
		AuthProviderName:  authProvider.Name(),
		StripePriceSingle: cfg.Billing.StripePriceSingle,
		StripePriceTeam:   cfg.Billing.StripePriceTeam,
	}, logger)
